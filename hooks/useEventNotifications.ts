...
export function useEventNotifications() {
     ...
     const checkForNewEvents = useCallback(async (currentEvents: Event[]) => {
          ...
          const newEvents = currentEvents.filter(
               (event) =>
                    !previousEventIds.has(event.eventId) && !lastNotifiedEvents.has(event.eventId)
          )
     ...
     const sendEventNotification = async (event: Event, type: "new" | "status_change") => {
          try {
