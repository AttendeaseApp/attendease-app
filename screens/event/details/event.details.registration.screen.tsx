...
export default function EventDetailsRegistrationScreen() {
  ...
  const isAbsent =
    registrationStatus?.isRegistered &&
    registrationStatus.attendanceStatus === "ABSENT";
  const isExcused =
    registrationStatus?.isRegistered &&
    registrationStatus.attendanceStatus === "EXCUSED";
  ...
  const handleRegister = async (faceData?: string) => {
    ...
    if (eventData?.eventStatus !== "ONGOING") {
      Alert.alert(
        "Event Not Available",
        "Registration is only available when the event is ongoing.",
      );
      return;
    }

    if (isFullyRegistered || isPartiallyRegistered || isAbsent || isExcused) {
    ...
    console.log("Starting registration with face:", !!faceData);
    ...
    performRegistration(faceData || null, async () => {
      try {
        ...
        if (
          updatedStatus.attendanceStatus === "PARTIALLY_REGISTERED" &&
          strictLocationValidation
        ) {
          startAutoUpgradePolling();
  ...
  useEffect(() => {
    if (
      ...
      console.log("Auto-registering with captured face data");
  ...
  const renderRegistrationButton = () => {
    if (isFullyRegistered) {
      const buttonText =
        registrationStatus?.attendanceStatus === "PRESENT"
          ? "Attended"
          : registrationStatus?.attendanceStatus === "LATE"
            ? "Registered (Late)"
            : "Registered";
    ...
    if (isAbsent || isExcused) {
      return (
        <Button disabled style={styles.infoButton}>
          <ButtonText>
            {registrationStatus?.message || "Marked as Absent"}
          </ButtonText>
        </Button>
      );
    }
        ...
        >
          ...
          {registrationStatus?.isRegistered && (
            <View
              style={[
                styles.statusBadge,
                isPartiallyRegistered && styles.partialBadge,
                isFullyRegistered && styles.successBadge,
                (isAbsent || isExcused) && styles.infoBadge,
              ]}
            >
                ...
                {eventData.eligibleStudents.allStudents ? (
                  ...
                  <View style={{ gap: 8 }}>
                    {eventData.eligibleStudents.cluster?.length &&
                      eventData.eligibleStudents.cluster.length > 0 && (
                        <View>
                          <ThemedText type="defaultSemiBold">
                            Clusters
                          </ThemedText>
                    ...
                    {eventData.eligibleStudents.course?.length &&
                      eventData.eligibleStudents.course.length > 0 && (
                        <View>
                          <ThemedText type="defaultSemiBold">
                            Courses
                          </ThemedText>
                    ...
                    {eventData.eligibleStudents.sections?.length &&
                      eventData.eligibleStudents.sections.length > 0 && (
                        <View>
          ...
          {strictLocationValidation && (
            <View style={styles.infoSection}>
              ...
              <ThemedText type="default">
                ...
                1. Check in at registration desk{"\n"}
                  ...
                  <View style={styles.pingStatusContainer}>
                    ...
                    <ThemedText type="default">
                      {trackingState.eventStatus?.includes("ongoing")
                        ...
                        : trackingState.eventStatus?.includes("not started") ||
                            trackingState.eventStatus?.includes("registration")
