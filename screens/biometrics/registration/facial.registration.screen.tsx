...
export default function OneTimeFacialRegistrationScreen() {
  ...
  const showAlert = (
    title: string,
    message: string,
    actions: typeof alertActions = [{ label: "OK" }],
  ) => {
    setAlertTitle(title);
  ...
  const sendImagesToServer = async (images: string[]) => {
    ...
    try {
      const result: BiometricsResult =
        await biometricsRegistrationService(images);
  ...
  if (!permission) {
    return (
      <View style={registrationScreenStyles.center}>
        ...
        <ThemedText
          type="default"
          style={registrationScreenStyles.permissionText}
        >
  ...
  return (
    <Animated.View
      style={[registrationScreenStyles.container, { opacity: fadeAnim }]}
    >
      <CameraView
