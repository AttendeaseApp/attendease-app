     ...
     getFacialStatus: async (): Promise<{
          ...
          try {
               ...
               if (!response.ok) {
                    ...
                    throw new Error(
                         errorData.message || "Failed to retrieve facial biometric status"
                    )
               }
          ...
          try {
               ...
               if (!response.ok) {
                    ...
                    if (response.status === 404 || response.status === 400) {
                         ...
                         throw new Error(
                              errorData.message ||
                                   "There is no facial biometric data in your account. " +
                                        "It might have already been deleted, or you haven't registered it yet."
                         )
                    }
                    const errorData = await response.json().catch(() => ({}))
