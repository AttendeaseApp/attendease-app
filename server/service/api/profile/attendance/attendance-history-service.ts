 ...
 * @param setLoading
 */
export async function getAllAttendanceHistory(setAttendanceHistories: React.Dispatch<React.SetStateAction<Event[]>>, setLoading: React.Dispatch<React.SetStateAction<boolean>>) {
    try {
        const response = await userAuthenticatedContextFetch(GET_ATTENDANCE_HISTORY_ENDPOINT);
