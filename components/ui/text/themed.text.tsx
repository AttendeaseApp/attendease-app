...
export function ThemedText({ style, type = "default", colorVariant = "black", fontFamilyOverride, fontWeightOverride, sizeMultiplier = 1, ...rest }: ThemedTextProps) {
    const { defaultFontFamily, theme } = useTheme();
    const t = typography[type];

    const color = colorVariant === "white" ? "#fff" : colorVariant === "gray" ? "#999" : theme.colors.text;
