...
export default function EventSessionsFeed() {
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            async function initWebSocket() {
                console.log("[EventFeed] Subscribing to WebSocket...");

                unsubscribe = await subscribeToEvents((newEvents) => {
                    console.log(
                        `[EventFeed] Received ${newEvents.length} events`,
                    );
                    setEvents(newEvents);
                ...
                initialLoadTimeout = setTimeout(() => {
                    console.log(
                        "[EventFeed] No initial broadcast received, showing empty state",
                    );
                    setLoadingEvents(false);
                }, 5000);
            }
            ...
            return () => {
                if (unsubscribe) {
                    console.log(
                        "[EventFeed] Cleaning up WebSocket subscription",
                    );
                    unsubscribe();
                }
                if (initialLoadTimeout) {
    ...
    const ongoingEvents = events.filter(
        (e) => e.eventStatus === EventStatus.ONGOING,
    );
    const upcomingEvents = events.filter(
        (e) => e.eventStatus === EventStatus.UPCOMING,
    );
    const registrationEvents = events.filter(
        (e) => e.eventStatus === EventStatus.REGISTRATION,
    );
                    ...
                    renderItem={({ item }) => (
                        <EventSessionCard
                            ...
                            facialVerificationEnabled={
                                item.facialVerificationEnabled
                            }
                            attendanceLocationMonitoringEnabled={
                                item.attendanceLocationMonitoringEnabled
                            }
                        />
                    )}
